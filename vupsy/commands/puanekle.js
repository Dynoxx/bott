...
if(sistemkontrol === 'kod')
{
  return message.reply(`Kod Doğrulama sistemi açık, bu komut bu sistemde kullanılamaz.`)
    ...
    if(sistemkontrol === 'public' || sistemkontrol === 'taglı')
    {
        ...
        if (!user) return message.reply(`Bir üye etiketlemelisin. => \`${prefix}puanekle @üye 10\``).then(x => x.delete({timeout: 5000}));
        let puan = args.filter(arg => !isNaN(arg))[0] || undefined;
        if(!user || !puan) return message.reply(`Hata: Lütfen tüm argümanları doldurunuz!  => \`${prefix}puanekle @üye 10\``).then(sil => sil.delete({timeout: 5000}));
        ...
        .setTitle("Puan Onayı")
        .setDescription(`${user} isimli üyeye **${puan}** sadakat puanı eklenecek onaylıyor musun?`)
        ...
        tepkibekle.delete()
            ...
            if(tepki.emoji.id == emojiconfig.onaylaid) {
                ...
                puanlog.send(`${message.author} tarafından ${user} isimli üyeye **${puan}** sadakat puanı eklendi.`)
                üyepuan.add(`üyesp_${kayitid}`,`${puan}`)
