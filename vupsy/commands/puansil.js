...
if(sistemkontrol === 'kod')
{
  return message.reply(`Kod Doğrulama sistemi açık, bu komut bu sistemde kullanılamaz.`)
    ...
    if(sistemkontrol === 'public' || sistemkontrol === 'taglı')
    {
        ...
        if (!user) return message.reply(`Bir üye etiketlemelisin. => \`${prefix}puansil @üye 10\``).then(x => x.delete({timeout: 5000}));
        let puan = args.filter(arg => !isNaN(arg))[0] || undefined;
        if(!user || !puan) return message.reply(`Hata: Lütfen tüm argümanları doldurunuz!  => \`${prefix}puansil @üye 10\``).then(sil => sil.delete({timeout: 5000}));
        ...
        .setTitle("Puan Onayı")
        .setDescription(`${user} isimli üyenin **${puan}** sadakat puanı silinecek onaylıyor musun?`)
        ...
        tepkibekle.delete()
            ...
            if(tepki.emoji.id == emojiconfig.onaylaid) {
                ...
                puanlog.send(`${message.author} tarafından ${user} isimli üyeden **${puan}** sadakat puanı silindi.`)
