  ...
  return message.reply(`Public Alım sistemi açık, bu komut bu sistemde kullanılamaz.`)
    ...
    if(sistemkontrol === 'taglı')
    {
      ...
      .setTitle("Kayıt Onayı")
        ...
        if (!user) return message.reply(`Bir üye etiketlemelisin. => \`${prefix}erkek @üye isim yaş\``).then(x => x.delete({timeout: 5000}));
        args = args.filter(a => a !== "" && a !== " ").splice(1);
        let isim = args.filter(arg => isNaN(arg)).map(arg => arg.charAt(0).replace('i', "İ").toUpperCase()+arg.slice(1)).join(" ");    
        if(!isim) return message.reply(`Bir isim girmelisin. => \`${prefix}erkek @üye isim\``);
        let BelirlenenIsim;
        let yaş = args.filter(arg => !isNaN(arg))[0] || undefined;
        if(!isim || !yaş) return message.reply(`Hata: Lütfen tüm argümanları doldurunuz!  => \`${prefix}erkek @üye isim yaş\``).then(sil => sil.delete({timeout: 5000}));
       ...
       await user.roles.set(rolconfig.ekiperkekrolleri);
        if(user.user.username.includes(serverconfig.tag)) {  user.roles.add(rolconfig.ekiprol);   }
         ...
         ydb.add(`yetkili_kayiterkek_${message.author.id}_`,1)
        ...
        const ilkcinsiyetcheck = tkdb.get(`tüye_ilkcinsiyet_${kayitid}_`)
         ...
         if(!ilkcinsiyetcheck)
          {
            tkdb.push(`tüye_ilkcinsiyet_${kayitid}_`,`Erkek`)
          }
        if(!kayityetkilicheck)
          ...
          kayitlog.send(`${user} ${message.author} tarafından  <@&${rolconfig.ekiperkekrol1}> Olarak Başarıyla Kaydedildi.`)
