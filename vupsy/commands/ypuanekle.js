...
if(sistemkontrol === 'kod')
{
  return message.reply(`Kod Doğrulama sistemi açık, bu komut bu sistemde kullanılamaz.`)
    ...
    if(sistemkontrol === 'public' || sistemkontrol === 'taglı')
    {
        ...
        if (!user) return message.reply(`Bir üye etiketlemelisin. => \`${prefix}ypuanekle @üye 10\``).then(x => x.delete({timeout: 5000}));
        ...
        if(!user || !puan) return message.reply(`Hata: Lütfen tüm argümanları doldurunuz!  => \`${prefix}ypuanekle @üye 10\``).then(sil => sil.delete({timeout: 5000}));
        ...
        .setTitle("Puan Onayı")
        .setDescription(`${user} isimli yetkiliye **${puan}** sadakat puanı eklenecek onaylıyor musun?`)
        ...
        tepkibekle.delete()
            ...
            if(tepki.emoji.id == emojiconfig.onaylaid) {
                ...
                message.channel.send(bilgiembed.setColor("GREEN").setTitle(`Puan Eklendi`).setDescription(`Puan verme işlemi başarılı :white_check_mark:`));
                puanlog.send(`${message.author} tarafından ${user} isimli yetkiliye **${puan}** sadakat puanı eklendi.`)
                ydb.add(`yetkili_puan${kayitid}`,`${puan}`)
        

            }
            if(tepki.emoji.id == emojiconfig.reddetid) {
                return  message.channel.send(bilgiembed.setColor("RED").setTitle(`İşlem İptal Edildi`).setDescription(`Puan Ekleme İşlemi İptal Edildi.`));
